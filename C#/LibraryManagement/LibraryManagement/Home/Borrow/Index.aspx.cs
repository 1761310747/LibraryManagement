    ...
    public partial class Index : System.Web.UI.Page
    {
        ...
        protected void submit_Click(object sender, EventArgs e)
        {
            ...
            if (borrow.Checked)
            {
                ...
                string a = "select times from Users where id = '"+ Session["id"].ToString().Trim() + "'";
                string b=con.RunSqlReturnString(a);
                string c = "select admin from Users where id = '"+ Session["id"].ToString().Trim() + "'";
                ...
                {
                    //插入借阅记录
                    string sql = "INSERT INTO Borrows ([book_id], [user_id], [borrow_date], [back_date]) VALUES ('" + bookId.Text.Trim() + "', '" + Session["id"].ToString().Trim() + "', '" + DateTime.Now.ToString() + "', '" + DateTime.Now.AddMonths(2).ToString() + "');";
                    //更新图书与用户借阅状态
                    string sql1 = "UPDATE Books SET times1 = times1+1 where id='" + bookId.Text.Trim() + "'";
                    string sql2 = "UPDATE Users SET times1 = times1+1,times = times-1 where id='" + Session["id"].ToString().Trim() + "'";
